...
Say("connected to target, starting service, starting bot")

#def fetch():
	#we need to define public / global variables to being set :|
	#text = update.message.text
	#len = update.message.entities.length
...
def bot_version_method(bot , update):
	...
	bot.send_message(chat_id = update.message.chat_id , text = "SIEGE ASSISTANT version 7.0")
...
def test_method (bot , update):
	Say("running test mothod, parsing data,")
...
def text_method (bot , update):
	#text = update.message.text
	#print(text)
	Say(update.message.text)
	print(update.message.text)
	leng = update.message.entities_length
	print(leng)
	print(update.message.entities_length)
...
updater.dispatcher.add_handler(CommandHandler('test' , test_method))
...
updater.dispatcher.add_handler(MessageHandler(Filters.text , text_method))
updater.dispatcher.add_handler(MessageHandler(Filters.reply , reply_method))#message_entities
